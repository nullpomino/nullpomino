...
　　play_slick.batをダブルクリックするとSlickバージョンが起動します。
...
　　netserver.batをダブルクリックするとネットプレイ用サーバーを起動します。

・Linux
　　端末ウィンドウでNullpoMinoがあるディレクトリ（このファイルがあるディレクトリ）まで移動して
　　以下のコマンドを入力してEnterキーを押すとたぶん起動します。（途中で改行しないように）
...
　　しかし、使用しているビデオカードやLinuxのバージョンによってはうまく動かないかもしれません。

　　Swingバージョン固有の問題:
　　　現時点ではまともに動きません。

　　Slickバージョン固有の問題:
　　　コマンドライン最初のXMODIFIERS=@im=noneは、使っているIMEがSCIM以外の場合は不要です。
...
E：ブロックの180度回転
F：エンディング早送り（SPPED MANIAとGARBAGE MANIAモードで使用可能）
QUIT：ゲームを終了する
PAUSE：ゲームを一時停止
GIVEUP：タイトルに戻る
RETRY：ゲームを最初からやり直す
...
【キー配置】
キー配置は初回起動時に設定します。
また、それ以降はタイトルの「CONFIG」の中にある「[KEYBOARD SETTING]」から変更できます。
...
STANDARD-ZERO　　　：STANDARD-PLUSから先行回転・先行ホールドを無くしてソフトドロップ速度を20Gにしたルール
　　　　　　　　　　 (Wojtekさん作・harddrop.com公式ルール)
...
・SPPED MANIA 2
　　SPPED MANIAを大きく超える速度でブロックが降ってくる上級者向けモードです。
...
・PHANTOM MANIA
　　SPPED MANIAモードと似ていますが、このモードでは置いたブロックが全く見えません。
...
追加したサーバーに接続する方法:
　1. 接続したいサーバーをリストボックスから選びます。(ダブルクリックすると即接続できます)
　2. 「接続」ボタンをクリックします。画面がロビー画面に切り替わります。
...
Windows:
java -cp NullpoMino.jar;lib\log4j-1.2.15.jar org.game_host.hebo.nullpomino.game.net.NetServer [ポート番号]
Linux/MacOS:
java -cp NullpoMino.jar:lib/log4j-1.2.15.jar org.game_host.hebo.nullpomino.game.net.NetServer [ポート番号]
...
SPPED MANIAとGARBAGE MANIAのエンディングでFボタンを押し続けるとエンディングの残り時間の短縮が可能に
